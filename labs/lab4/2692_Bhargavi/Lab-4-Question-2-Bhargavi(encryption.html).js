...
<script>
    ...
    async function storeTasks() {
        ...
        const encryptedBlob = new Blob([new Uint8Array(encryptedArray)]);

        store.put({ id: 1, data: encryptedBlob });
        ...
        transaction.onerror = (event) => {
            console.error("Error storing encrypted tasks:", event.target.error);
        };
    }
    ...
    async function readTasks() {
        ...
        request.onsuccess = async (event) => {
            const encryptedBlob = event.target.result.data;
